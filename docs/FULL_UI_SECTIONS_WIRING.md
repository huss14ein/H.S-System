...
|--------|--------|----------------|
| **Period Financial Report** | Browser Print → Save as PDF (+ JSON) | Modal hosted in **`Layout`** (`PeriodFinancialReportModal`); opens via `openPeriodFinancialReportModal()` / `finova:open-period-financial-report`. Model: `buildPeriodFinancialReportModel` (`periodFinancialReportModel.ts`) over FY/CY/YTD/12M/custom + prior twin (`periodReportWindow.ts`); print HTML: `generatePeriodFinancialReportHtml` (sections 1→12 + orphans + appendix, TOC, SVG charts). Portfolio window P/L: `computePortfolioPnLForWindow`. Window-scoped: cashflow, cards, portfolio P/L, budget engine preset, installment schedules (`fetchPeriodReportInstallmentSnapshot`). As-of-today (labeled): headline NW, ROI, Dashboard↔Summary recon via `reconcileDashboardVsSummaryKpis`. Validations: `validatePeriodReportRequest` (data, preset, custom range bounds, ≥1 section). Modal: section picker, JSON export, `yieldToMain` while building (no UI freeze), stays open if print blocked. **No binary PDF library.** |
