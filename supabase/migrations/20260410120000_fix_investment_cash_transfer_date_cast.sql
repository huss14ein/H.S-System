-- Fix type mismatch when inserting RPC p_date into transactions.date (date column).
-- Previous function cast p_date to text, which fails on schemas where transactions.date is DATE.
