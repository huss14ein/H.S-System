...
const AllocationBarChart: React.FC<AllocationBarChartProps> = ({ data }) => {
    ...
    const totalValue = useMemo(() => sanitizedData.reduce((sum, item) => sum + item.value, 0), [sanitizedData]);
    const chartData = [...sanitizedData].reverse();
    const isEmpty = !chartData.length;

    return (
        <ChartContainer height="100%" isEmpty={isEmpty} emptyMessage="No asset-class allocation data available.">
