    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3">
          <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1">
            Rebound needed
            <InfoHint text="Rally from today’s price back to the new average. Compare with “if you hold” — that is the bounce to the old average with no add." />
          </p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3">
          <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1">
            Cash to deploy
            <InfoHint text="Planned ladder spend in this position’s book currency. Recovery budget is shared — lower-priority names wait or recycle." />
          </p>
          <p className="mt-1 text-lg font-black tabular-nums text-slate-900">
            {decision.action === 'add_ladder' ? formatMoney(m.cashToDeploy) : '0'}
          </p>
          <p className="text-xs text-slate-500">{bookCurrency} · {m.sharesToAdd.toFixed(2)} shares</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-3">
          <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1">
            First buy
            <InfoHint text="First limit below last. Place this rung before the rest of the ladder." />
          </p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3">
          <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1">
            Extra loss if −10%
            <InfoHint text="Paper loss on the new cash only if price falls another 10% after you add. Does not include the existing position." />
          </p>
          <p className="mt-1 text-lg font-black tabular-nums text-rose-800">
            {decision.action === 'add_ladder' ? formatMoney(m.extraLossIfDown10) : '—'}
          </p>
