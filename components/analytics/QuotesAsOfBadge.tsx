...
import { isQuotesFromLiveApi, quoteSourceDisplayLabel } from '../../services/quoteSessionStatus';

export const QuotesAsOfBadge: React.FC<{ className?: string }> = ({ className = '' }) => {
  const { quotesPriceSource, symbolQuoteUpdatedAt, isRefreshing, hasQueuedPriceRefresh } = useMarketQuoteMeta();
  const { t, language } = useLanguage();
  const quotesLive = isQuotesFromLiveApi(quotesPriceSource);

  const label = useMemo(() => {
    if (isRefreshing || hasQueuedPriceRefresh()) return t('quotesRefreshing');
    if (quotesPriceSource === 'none') return quoteSourceDisplayLabel('none');
    if (quotesPriceSource === 'cached') return t('quotesCached');
  ...
  }, [quotesPriceSource, isRefreshing, hasQueuedPriceRefresh, symbolQuoteUpdatedAt, language, t]);
    ...
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${
          quotesLive && !isRefreshing ? 'bg-emerald-500' : quotesPriceSource === 'cached' ? 'bg-sky-500' : 'bg-amber-500'
        }`}
      />
      {label}
    </span>
