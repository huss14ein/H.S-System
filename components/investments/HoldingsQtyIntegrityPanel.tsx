...
function anyIntegrityBusy(rebuildBusyKey: string | null, ackBusyKey: string | null): boolean {
  return Boolean(rebuildBusyKey || ackBusyKey);
}
...
const HoldingsQtyIntegrityPanel: React.FC<Props> = ({ compact = false, onReconcileQuantity, onAlignLotsToBook }) => {
  ...
  const integrityFp = useMemo(
    () => buildHoldingsIntegrityFingerprint(data),
    [data?.investments, data?.investmentTransactions, data?.reconciliationAdjustments, data?.corporateActionEvents],
  );
  ...
  const keepStored = (r: HoldingsQtyDriftRow) => {
    const ticker = safeTickerLabel(r.symbol);
    const key = integrityBusyKey(r.portfolioId, r.symbol);
    if (anyIntegrityBusy(rebuildBusyKey, ackBusyKey)) return;
    setAckBusyKey(key);
    void (async () => {
      try {
        ...
        setAcks(next);
        toast(
          `Kept stored ${ticker}: ${r.storedQuantity.toLocaleString()} shares — this warning will not return until the book or ledger changes.`,
          'success',
        );
      } catch (err) {
  ...
  const keepClosed = (r: { portfolioId: string; symbol: string; ledgerNet: number }) => {
    ...
    void (async () => {
      try {
        ...
        setAcks(next);
        toast(`Kept ${ticker} closed — will not re-open from ledger.`, 'success');
      } catch (err) {
        toast(err instanceof Error ? err.message : 'Could not save dismissal.', 'error');
      } finally {
        setAckBusyKey(null);
  ...
  const rebuild = async (
    ...
    try {
      ...
      if ((opts?.restoreOpen || opts?.reopenSold) && storedAfter <= 1e-9) {
        throw new Error(
          `Restore did not create an open ${ticker} holding from the ledger. Check that buys are tagged to this portfolio, then try again.`,
        );
      }
      ...
      toast(
        opts?.restoreOpen
          ? `Restored ${ticker} holding from ledger (${storedAfter.toLocaleString()} shares).`
          : opts?.reopenSold
            ? `Re-opened ${ticker} from ledger.`
            : `Rebuilt ${ticker} from ledger — warning dismissed until book or ledger changes.`,
        'success',
      );
    } catch (err) {
    ...
    try {
      ...
      for (const [portfolioId, symbols] of byPortfolio) {
        ...
        for (const symbol of symbols) {
          ...
          if (storedAfter <= 1e-9) {
            throw new Error(
              `Restore did not create an open ${safeTickerLabel(symbol)} holding from the ledger. Check portfolio-tagged buys, then retry.`,
            );
          }
          restoredCount += 1;
      ...
      >
        ...
        <span className="flex gap-2 relative z-10">
          <button
            ...
            disabled={anyIntegrityBusy(rebuildBusyKey, ackBusyKey)}
          ...
          <button
            ...
            disabled={anyIntegrityBusy(rebuildBusyKey, ackBusyKey) || !ctx?.rebuildHoldingsFromLedgerForSymbols}
            className={
              mode === 'open'
            ...
            return (
              ...
              >
                ...
                <span className="flex gap-2 relative z-10">
                  ...
                  >
                    {ackBusyKey === busyKey ? 'Saving…' : 'Keep stored'}
                  </button>
                  {onReconcileQuantity && (
                    <button
                      type="button"
                      ...
                      disabled={anyIntegrityBusy(rebuildBusyKey, ackBusyKey)}
                      className="text-xs px-2.5 py-1.5 rounded-md border border-emerald-400 text-emerald-900 bg-emerald-50 hover:bg-emerald-100 font-medium cursor-pointer disabled:opacity-50"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                  ...
                  >
                    {rebuildBusyKey === busyKey ? 'Rebuilding…' : 'Rebuild this symbol'}
                  </button>
                </span>
