...
const generateRealisticData = (): FinancialData => {
  ...
  while (currentDate <= endDate) {
    ...
    if (day === 2) {
        if (month === 0 || month === 6) transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc1', date: currentDate.toISOString().split('T')[0], description: 'Rent (Biannual)', amount: -18000, category: 'Rent', budgetCategory: 'Housing', type: 'expense', expenseType: 'Core' });
        if (month === 5) transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc1', date: currentDate.toISOString().split('T')[0], description: 'Dependents Fees', amount: -2000, category: 'Iqama Fees', budgetCategory: 'Work and Residency-Related Expenses', type: 'expense', expenseType: 'Core' });
        if (month === 5 || month === 10) transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc4', date: currentDate.toISOString().split('T')[0], description: 'Vacation Trip', amount: -8000, category: 'Travel', budgetCategory: 'Travel and Vacation', type: 'expense', expenseType: 'Discretionary' });
        if (month === 7) transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc1', date: currentDate.toISOString().split('T')[0], description: 'Tuition Fees', amount: -5500, category: 'School Fees', budgetCategory: 'Education', type: 'expense', expenseType: 'Core' });
        transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc1', date: currentDate.toISOString().split('T')[0], description: 'STC Bill', amount: -(250 + Math.random() * 50), category: 'Internet Bill', budgetCategory: 'Communication', type: 'expense', expenseType: 'Core' });
        transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc2', date: currentDate.toISOString().split('T')[0], description: 'Investment Contribution', amount: -5000, category: 'Brokerage Deposit', budgetCategory: 'Savings and Investments', type: 'expense', expenseType: 'Core' });
    }
    if ([5, 12, 19, 26].includes(day)) {
        transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc1', date: currentDate.toISOString().split('T')[0], description: 'Lulu Groceries', amount: -(500 + Math.random() * 100), category: 'Groceries', budgetCategory: 'Food and Groceries', type: 'expense', expenseType: 'Core' });
        transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc1', date: currentDate.toISOString().split('T')[0], description: 'Gasoline', amount: -(150 + Math.random() * 50), category: 'Fuel', budgetCategory: 'Transportation', type: 'expense', expenseType: 'Core' });
    }
    if ([10, 24].includes(day)) {
       transactions.push({ id: `txn${txnIdCounter++}`, accountId: 'acc4', date: currentDate.toISOString().split('T')[0], description: 'Dinner Out', amount: -(250 + Math.random() * 100), category: 'Restaurants', budgetCategory: 'Food and Groceries', type: 'expense', expenseType: 'Discretionary' });
    }
