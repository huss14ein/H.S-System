...
export function generateWealthMetricPassportHtml(
  ...
  if (metric === 'netWorth') {
    ...
    sectionC = `Present value ${money(model.investmentsTotalSar)}. Net invested after withdrawals ${money(model.investmentNetInvestedSar)}. Growth ${money(model.investmentGrowthSar)}${model.investmentPrincipalFullyRecovered ? ' (principal recovered — leftover value is profit)' : ''}. Same path: computeHeadlinePersonalInvestmentRoiDecimal.`;
