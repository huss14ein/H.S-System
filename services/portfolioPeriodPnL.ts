...
export type PortfolioWindowPnLRow = {
  ...
  bookCurrency: ReturnType<typeof resolveInvestmentPortfolioCurrency>;
  valueSar: number;
  window: PortfolioPeriodPnLBreakdown;
};

export type PortfolioWindowPnLResult = {
...
/**
 * Portfolio P/L for an arbitrary [start, end] window (Period Financial Report).
 * Same attribution / MTM math as week+month summary cards.
 */
export function computePortfolioPnLForWindow(args: {
  ...
  for (const [, siblings] of byAccount) {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      const windowPnL = computePortfolioMarkToMarketPeriodPnLSar({
        portfolio: p,
        transactions: txsForLedger,
        startMs,
        endMs,
  ...
  rows.sort((a, b) => Math.abs(b.window.totalSar) - Math.abs(a.window.totalSar));
  return {
    rows,
    totalSar: rows.reduce((s, r) => s + r.window.totalSar, 0),
    ledgerSar: rows.reduce((s, r) => s + r.window.ledgerSar, 0),
    marketEstimateSar: rows.reduce((s, r) => s + r.window.marketEstimateSar, 0),
    startMs,
    endMs,
  };
}
