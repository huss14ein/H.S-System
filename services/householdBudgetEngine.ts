...
export interface HouseholdEngineConfig {
  ...
  utilities: { baseMonthly: number; perAdultLoad: number; perKidLoad: number };
  transport: { singleDriverBaseCost: number; rideSupportEnabled: boolean; rideSupportCost: number };
  allowances: { spouseFixedEnabled: boolean; spouseFixedAmount: number; personalAllowanceEnabled: boolean; personalAllowancePerAdult: number };
  obligations: { annual: number; semiAnnual: number; monthlyFixed: number; annualDueMonth?: number; semiAnnualDueMonths?: number[] };
  requiredExpenses: {
    annualReserveEnabled: boolean; annualReserveAmount: number;
    semiAnnualReserveEnabled: boolean; semiAnnualReserveAmount: number;
    monthlyRequiredEnabled: boolean; monthlyRequiredAmount: number;
  };
...
export interface HouseholdEngineResult {
  ...
  plannedVsActual: {
    ...
    plannedOutflow: number; actualOutflow: number;
    plannedNet: number; actualNet: number;
  };
  balanceProjection: { openingLiquid: number; projectedYearEndLiquid: number };
}
...
export const DEFAULT_HOUSEHOLD_ENGINE_CONFIG: HouseholdEngineConfig = {
  ...
  utilities: { baseMonthly: 420, perAdultLoad: 110, perKidLoad: 65 },
  transport: { singleDriverBaseCost: 900, rideSupportEnabled: false, rideSupportCost: 0 },
  allowances: { spouseFixedEnabled: true, spouseFixedAmount: 1200, personalAllowanceEnabled: false, personalAllowancePerAdult: 250 },
  obligations: { annual: 12000, semiAnnual: 6000, monthlyFixed: 0, annualDueMonth: 12, semiAnnualDueMonths: [6, 12] },
  requiredExpenses: {
    annualReserveEnabled: true, annualReserveAmount: 6000,
    semiAnnualReserveEnabled: true, semiAnnualReserveAmount: 3600,
    monthlyRequiredEnabled: true, monthlyRequiredAmount: 300,
  },
...
export const HOUSEHOLD_ENGINE_PROFILES: Record<HouseholdEngineProfile, { label: string; description: string; config: Partial<HouseholdEngineConfig> }> = {
  ...
  Moderate: {
    ...
    config: { operatingMode: 'Balanced', emergencyTargetMonths: 6, reserveTargetMonths: 2, bucketRules: DEFAULT_HOUSEHOLD_ENGINE_CONFIG.bucketRules },
  },
  Growth: {
    label: 'Growth',
...
export function inferObligationsFromTransactions(
  ...
  (transactions || []).forEach((t) => {
    ...
    byMonth[d.getMonth()] += Math.abs(Number(t.amount) || 0);
  });
...
export function buildHouseholdEngineInputFromData(
  ...
  const monthlySalaryPlan = incomeByMonth.map((v) => (v > 0 ? v : salaryFallback));
  const profileConfig = HOUSEHOLD_ENGINE_PROFILES[profile]?.config ?? HOUSEHOLD_ENGINE_PROFILES.Moderate.config;
  const autoGoalPriority = goalsForRouting.length > 0 ? [...goalsForRouting].sort((a, b) => b.remaining - a.remaining).map((g) => g.name) : undefined;
  const inferred = inferObligationsFromHistory && (transactions?.length ?? 0) > 0 ? inferObligationsFromTransactions(transactions as never[], year) : {};
  return {
    monthlySalaryPlan,
...
export function buildHouseholdEngineInputFromPlanData(
  ...
  const avgIncome = pastIncome.length > 0 ? pastIncome.reduce((a, b) => a + b, 0) / pastIncome.length : 0;
  const salaryFallback = expectedMonthlySalary && expectedMonthlySalary > 0 ? expectedMonthlySalary : avgIncome;
  const monthlySalaryPlan = planned.map((v, idx) => (v > 0 ? v : actualInc[idx] > 0 ? actualInc[idx] : salaryFallback));
  const profileConfig = HOUSEHOLD_ENGINE_PROFILES[profile]?.config ?? HOUSEHOLD_ENGINE_PROFILES.Moderate.config;
  const autoGoalPriority = goalsForRouting.length > 0 ? [...goalsForRouting].sort((a, b) => b.remaining - a.remaining).map((g) => g.name) : undefined;
  return {
    monthlySalaryPlan,
...
export const HOUSEHOLD_ENGINE_SAMPLE_SCENARIOS: Array<{ id: string; label: string; defaults: HouseholdComposition; overrides: HouseholdMonthlyOverride[]; config?: Partial<HouseholdEngineConfig> }> = [
  { id: 'normal-family', label: 'Normal year (2 adults, 2 kids)', defaults: { adults: 2, kids: 2 }, overrides: [{ month: 9, kids: 3 }] },
  {
    id: 'pressure-year',
    label: 'High-pressure year (income dip + cost spike)',
    defaults: { adults: 2, kids: 1 },
    overrides: [{ month: 3, salary: 0 }, { month: 4, salary: 0 }, { month: 7, fixedObligations: 6000, essentials: 7000 }, { month: 8, fixedObligations: 6000, essentials: 7000 }],
    config: { transport: { singleDriverBaseCost: 1200, rideSupportEnabled: true, rideSupportCost: 550 } },
  },
  { id: 'goal-routing', label: 'Goal routing (house first then car)', defaults: { adults: 2, kids: 0 }, overrides: [], config: { autoRouteGoalPriority: ['House', 'Car', 'Travel'] } },
];
...
function mergeConfig(input?: Partial<HouseholdEngineConfig>): HouseholdEngineConfig {
  const merged: HouseholdEngineConfig = {
    ...
    ...input,
    utilities: { ...DEFAULT_HOUSEHOLD_ENGINE_CONFIG.utilities, ...(input?.utilities || {}) },
    ...
    requiredExpenses: { ...DEFAULT_HOUSEHOLD_ENGINE_CONFIG.requiredExpenses, ...(input?.requiredExpenses || {}) },
    bucketRules: { ...DEFAULT_HOUSEHOLD_ENGINE_CONFIG.bucketRules, ...(input?.bucketRules || {}) },
    autoRouteGoalPriority: Array.isArray(input?.autoRouteGoalPriority) && input.autoRouteGoalPriority.length > 0 ? input.autoRouteGoalPriority : DEFAULT_HOUSEHOLD_ENGINE_CONFIG.autoRouteGoalPriority,
  };
...
export function buildHouseholdBudgetPlan(input: HouseholdEngineInput): HouseholdEngineResult {
  ...
  const annualBuckets: Record<HouseholdBucketKey, number> = {
    fixedObligations: 0, householdEssentials: 0, householdOperations: 0, transport: 0, personalSupport: 0,
    reserveSavings: 0, emergencySavings: 0, goalSavings: 0, kidsFutureSavings: 0, retirementSavings: 0, investing: 0,
  };
  ...
  const months: HouseholdMonthPlan[] = Array.from({ length: 12 }, (_, index) => {
    ...
    buckets.householdOperations = config.bucketRules.householdOperations.enabled ? toCurrency(Math.max(salary * (config.bucketRules.householdOperations.minPctOfSalary || 0), 400 + kids * 140) + monthlyRequiredOther + utilityLoad + unusualExtra) : 0;
    ...
    if (remaining < 0) {
      const cut = (key: HouseholdBucketKey) => {
        ...
        if (current <= 0) return;
        const reduce = Math.min(current, Math.abs(remaining));
    ...
    if (dueDeduction > 0) {
      ...
      if (reservePoolBalance < 0) warnings.push(`Reserve pool underfunded after due-month deduction (${dueDeduction.toLocaleString()}).`);
    }
    if (salary <= 0) warnings.push('No salary planned: non-essential allocations are auto-frozen.');
    const pressureRatio = salary > 0 ? (Object.values(buckets).reduce((s, v) => s + v, 0) + routedGoalAmount) / salary : 1;
    if (pressureRatio > 1) warnings.push('Affordability pressure: required allocations exceed salary.');
    if (remainingEmergencyGap > 0 && month > 6) warnings.push('Emergency target still behind after mid-year.');
    const totalPlannedOutflow = toCurrency(Object.values(buckets).reduce((s, v) => s + v, 0) + routedGoalAmount);
    if (salary < Object.values(buckets).reduce((s, v) => s + v, 0)) validationErrors.push('Over-allocation: protected + base exceed salary.');
    if (salary - totalPlannedOutflow < 0) validationErrors.push('Negative remainder for this month.');
    if (remainingReserveGap > 0 && month > 6) validationErrors.push('Reserve gap underfunded after mid-year.');
    const incomeActual = monthlyActualIncome[index];
    const totalActualOutflow = monthlyActualExpense[index];
    return {
      month, adults, kids,
      incomePlanned: salary, incomeActual, totalPlannedOutflow, totalActualOutflow,
      plannedNet: round2(salary - totalPlannedOutflow), actualNet: round2(incomeActual - totalActualOutflow),
      routedGoalName, routedGoalAmount, reservePoolAfterDeductions: reservePoolBalance,
      buckets, validationErrors, warnings,
    };
  });
  ...
  if (remainingEmergencyGap > 0) recommendations.push(`Emergency fund short by ${round2(remainingEmergencyGap).toLocaleString()}. Keep emergency bucket protected.`);
  if (remainingEmergencyGap <= 0 && (input.monthlyActualIncome?.some((v) => v > 0) ?? false)) recommendations.push('Emergency fund target on track. Keep contributing to maintain the buffer.');
  ...
  if (months.some((m) => (m.routedGoalAmount + m.buckets.goalSavings) > 0 && m.routedGoalName)) recommendations.push('Surplus is auto-routed to your top-priority goal. Adjust goal order in Goals to change priority.');
  ...
  return {
    ...
    balanceProjection: { openingLiquid: toCurrency(input.liquidBalance), projectedYearEndLiquid: toCurrency(input.liquidBalance + plannedNet) },
  };
}
