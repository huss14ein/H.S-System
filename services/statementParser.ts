...
export function extractInvestmentTransactionsFromStructuredText(text: string, accountId: string): InvestmentTransaction[] {
  ...
  for (const line of lines) {
    ...
    if (buy) {
      ...
      const currency = String(buy[3] || 'USD').toUpperCase() === 'USD' ? 'USD' : 'SAR';
      const price = parseNumber(buy[4]);
    ...
    if (!parsed && sell) {
      ...
      const currency = String(sell[3] || 'USD').toUpperCase() === 'USD' ? 'USD' : 'SAR';
      const price = parseNumber(sell[4]);
