...
describe('tradeHoldingsIsolation', () => {
  ...
  it('DataContext trade path uses applyPositionDeltaForTrade + syncLotsAfterTrade (not full-book persist)', () => {
    ...
    const buySellStart = ctx.lastIndexOf("if (tradeData.type === 'buy' || tradeData.type === 'sell')", posIdx);
    const buySellEnd = ctx.indexOf('const runLotSync = async', posIdx);
    expect(buySellEnd).toBeGreaterThan(posIdx);
    const buySellBlock = ctx.slice(buySellStart, buySellEnd);
    expect(buySellBlock).toContain('applyPositionDeltaForTrade');
    expect(buySellBlock).toContain('await applyInvestmentAccountDeltaForTrade');
    expect(buySellBlock).toContain('await applyPositionDeltaForTrade');
