...
describe('structured trading statement parser', () => {
  ...
  it('parses fee and VAT cash-impact rows without position changes', () => {
    const text = [
      'Transaction Statement (SAR)',
      '24/01/2026 10:00:00  Ordf 50000001  Purchase of Security 68.00 2223.SR @ SAR 86.90  5,909.20000    0.00000',
      '24/01/2026 10:00:01  Ordf 50000002  Trade Fees 2223.SR  2.95460  0.00000',
      '24/01/2026 10:00:02  Ordf 50000003  VAT 2223.SR  0.17728  0.00000',
    ].join('\n');

    const rows = extractInvestmentTransactionsFromAwaedTable(text, 'acc-1', { statementCurrency: 'SAR' });
    expect(rows.length).toBe(3);
    expect(rows.find((r) => r.type === 'buy')?.total).toBeCloseTo(5909.2);
    expect(rows.find((r) => r.type === 'fee')?.total).toBeCloseTo(2.9546);
    expect(rows.find((r) => r.type === 'vat')?.total).toBeCloseTo(0.17728);
    expect(rows.filter((r) => r.type === 'fee' || r.type === 'vat').every((r) => r.quantity === 0)).toBe(true);
  });

  it('reads debit/credit columns correctly when statement lines include trailing debit credit balance', () => {
    const text = [
      'Transaction Statement (SAR)',
      '24/01/2026 10:00:00  Ordf 50000001  Purchase of Security 68.00 2223.SR @ SAR 86.90   5,909.20000   0.00000   10,000.00000',
      '25/01/2026 10:00:00  Ordf 50000002  Sale of Security 10.00 2223.SR @ SAR 90.00       0.00000       900.00000  10,900.00000',
    ].join('\n');

    const rows = extractInvestmentTransactionsFromStructuredText(text, 'acc-1', { statementCurrency: 'SAR' });
    expect(rows.length).toBe(2);
    expect(rows.find((r) => r.type === 'buy')?.total).toBeCloseTo(5909.2, 5);
    expect(rows.find((r) => r.type === 'sell')?.total).toBeCloseTo(900, 5);
  });
});
