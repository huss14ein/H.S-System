...
const UpcomingBills: React.FC = () => {
    ...
    const bills = [
        { name: 'Mortgage Payment', date: '2024-09-01', amount: 5500 },
        { name: 'Car Loan', date: '2024-09-05', amount: 1200 },
        { name: 'Netflix Subscription', date: '2024-09-10', amount: 50 },
    ];
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
...
const Dashboard: React.FC<{ setActivePage: (page: Page) => void }> = ({ setActivePage }) => {
    ...
    const { kpiSummary, monthlyBudgets, investmentTreemapData } = useMemo(() => {
        ...
        monthlyTransactions.filter(t => t.type === 'expense').forEach(t => {
                const currentSpend = monthlySpending.get(t.category) || 0;
                monthlySpending.set(t.category, currentSpend + Math.abs(t.amount));
            });

        const monthlyBudgets = data.budgets.map(budget => ({
            ...budget,
        ...
        return {
            kpiSummary: {
                ...
                budgetVariance,
                roi,
                assetMix: [], liquidNetWorth: 0, liabilitiesCoverage: 0, monthlyIncome:0, monthlyExpenses:0,
            },
            monthlyBudgets,
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card title="Net Worth" value={formatCurrencyString(kpiSummary.netWorth)} trend="+2.5%" onClick={() => setActivePage('Summary')} />
                ...
                <Card title="Total Investment ROI" value={`${(kpiSummary.roi * 100).toFixed(1)}%`} trend="+8.7%" tooltip="Return on Investment based on total capital invested." onClick={() => setActivePage('Investments')} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                 <div className="lg:col-span-3 bg-white p-6 rounded-lg shadow-md hover:shadow-xl hover:scale-[1.01] transition-all duration-300 ease-in-out cursor-pointer" onClick={() => setActivePage('Transactions')}>
                    ...
                    <div className="h-80"><CashflowChart /></div>
                 </div>
