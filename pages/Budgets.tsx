...
const Budgets: React.FC = () => {
    ...
    const criticalValidationCount = useMemo(() => householdBudgetEngine.months.reduce((sum: number, m: { validationErrors?: string[] }) => sum + ((m.validationErrors?.length || 0) > 0 ? 1 : 0), 0), [householdBudgetEngine]);
            ...
            <SectionCard title="Household Budget Engine">
                ...
                <div className="mt-4 flex flex-wrap gap-4 items-end">
                    <div>
                        ...
                        <select value={engineProfile} onChange={(e) => setEngineProfile(e.target.value as HouseholdEngineProfile)} className="p-2 border border-slate-200 rounded-lg bg-white text-sm min-w-[140px]">
                    ...
                    <div>
                        ...
                        <input type="number" min={0} step={100} value={expectedMonthlySalary} onChange={(e) => setExpectedMonthlySalary(e.target.value === '' ? '' : Number(e.target.value))} placeholder={suggestedMonthlySalary ? `Auto: ${formatCurrencyString(suggestedMonthlySalary, { digits: 0 })}` : 'From transactions'} className="p-2 border border-slate-200 rounded-lg w-36 text-sm" />
                ...
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <div className="rounded-lg border border-slate-200 p-3 bg-slate-50"><p className="text-xs text-slate-500">Planned annual net</p><p className="font-bold text-slate-900">{formatCurrencyString(householdBudgetEngine.plannedVsActual.plannedNet, { digits: 0 })}</p></div>
                    <div className="rounded-lg border border-slate-200 p-3 bg-slate-50"><p className="text-xs text-slate-500">Actual annual net</p><p className="font-bold text-slate-900">{formatCurrencyString(householdBudgetEngine.plannedVsActual.actualNet, { digits: 0 })}</p></div>
                    <div className="rounded-lg border border-emerald-200 p-3 bg-emerald-50"><p className="text-xs text-slate-600">Projected year-end liquid</p><p className="font-bold text-emerald-700">{formatCurrencyString(householdBudgetEngine.balanceProjection.projectedYearEndLiquid, { digits: 0 })}</p></div>
                    <div className="rounded-lg border border-indigo-200 p-3 bg-indigo-50"><p className="text-xs text-slate-600">Auto-routed goal</p><p className="font-bold text-indigo-700">{householdBudgetEngine.months.find((m: HouseholdMonthPlan) => m.routedGoalName)?.routedGoalName || 'None'}</p></div>
                </div>
                ...
                {criticalValidationCount > 0 && (
                    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3">
                        ...
                        <ul className="mt-1 list-disc pl-5 text-xs text-amber-800 space-y-0.5">
                            {householdBudgetEngine.months.filter((m: { validationErrors?: string[] }) => (m.validationErrors?.length || 0) > 0).slice(0, 3).map((m: { month: number; validationErrors?: string[] }) => (
                ...
                <details className="mt-4 rounded-lg border border-slate-200 bg-slate-50">
                    ...
                    <div className="p-3 pt-0 space-y-3">
                        <div className="flex flex-wrap gap-2">
                            {HOUSEHOLD_ENGINE_SAMPLE_SCENARIOS.map((scenario: { id: string; label: string; defaults: { adults: number; kids: number }; overrides: HouseholdMonthlyOverride[] }) => (
                                <button key={scenario.id} type="button" className={`px-3 py-1.5 rounded-lg border text-sm ${selectedScenario === scenario.id ? 'bg-primary text-white border-primary' : 'bg-white border-slate-200 text-slate-700'}`} onClick={() => { setSelectedScenario(scenario.id); setHouseholdAdults(scenario.defaults.adults); setHouseholdKids(scenario.defaults.kids); setHouseholdOverrides(scenario.overrides); }}>
                                    {scenario.label}
                                </button>
                            ))}
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full text-xs">
                                <thead><tr className="text-slate-500 border-b"><th className="text-left py-1 pr-2">Month</th><th className="text-left py-1 pr-2">Salary</th><th className="text-left py-1 pr-2">Adults</th><th className="text-left py-1 pr-2">Kids</th></tr></thead>
                                <tbody>
